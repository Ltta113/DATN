...
<div class="bg-white rounded-lg p-6 mt-6">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        ...
        <div class="bg-green-50 rounded-lg p-4 shadow-sm border border-green-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-green-600 font-medium">Sách Đã Bán</p>
                    <h3 class="text-2xl font-bold text-gray-800">{{ number_format($totalSoldBooks) }}</h3>
                </div>
        ...
        <!-- Tổng số sách -->
        <div class="bg-yellow-50 rounded-lg p-4 shadow-sm border border-yellow-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-yellow-600 font-medium">Tổng Số Sách</p>
                    <h3 class="text-2xl font-bold text-gray-800">{{ number_format($totalStockBooks + $totalSoldBooks) }}
                    </h3>
                </div>
    ...
    <!-- Phần Doanh Thu -->
    <h3 class="text-xl font-semibold text-gray-700 mb-4">Tổng Quan Doanh Thu</h3>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
            <p class="text-sm text-gray-500 font-medium">Tháng Trước</p>
            <h3 class="text-2xl font-bold text-gray-800 mt-2">{{ number_format($totalRevenueLastMonth, 0) }} VNĐ</h3>
        </div>

        <!-- Tổng doanh thu -->
        <div class="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
            <p class="text-sm text-gray-500 font-medium">Tổng Doanh Thu</p>
            <h3 class="text-2xl font-bold text-gray-800 mt-2">{{ number_format($totalRevenueAllTime, 0) }} VNĐ</h3>
            <div class="mt-1 text-xs text-gray-500">
                @if ($totalSoldBooks > 0)
                    Trung bình: {{ number_format($totalRevenueAllTime / $totalSoldBooks, 0) }} VNĐ/sách
                @endif
            </div>
        </div>
    </div>

    <!-- So sánh doanh thu hàng tháng -->
