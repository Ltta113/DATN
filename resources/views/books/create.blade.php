    ...
    <div class="max-w-5xl mx-auto p-6 bg-white shadow-md rounded-lg">
        ...
        <form action="{{ route('admin.books.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- ISBN -->
                <div>
                    <label for="isbn" class="block text-left text-gray-700 mb-2">
                    ...
                    <div class="bg-gray-100 rounded-lg flex items-center">
                        ...
                        <input type="text" id="isbn" name="isbn" value="{{ old('isbn') }}" required
                            class="bg-transparent w-full py-3 px-4 text-left outline-none" placeholder="Nhập mã ISBN">
                    </div>
                </div>
                ...
                <!-- Ngôn ngữ -->
                <div>
                    <label for="language" class="block text-left text-gray-700 mb-2">
                        Ngôn ngữ
                    </label>
                    <div class="bg-gray-100 rounded-lg flex items-center">
                        <svg class="w-5 h-5 text-gray-500 mx-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9M9 15l3-3m0 0l-3-3m3 3H9"></path>
                        </svg>
                        <input type="text" id="language" name="language" value="{{ old('language') }}"
                            class="bg-transparent w-full py-3 px-4 text-left outline-none"
                            placeholder="Nhập ngôn ngữ của sách">
                    </div>
                </div>
            ...
            <!-- Tác giả - UI kiểu tags full width -->
            <div class="mt-6">
            ...
            <!-- Thể loại - UI kiểu tags full width -->
            <div class="mt-6">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function setupMultiSelect(type) {
                ...
                // Xử lý các tag đã có sẵn (từ old values)
                const existingTags = tagsContainer.querySelectorAll('.tag-item');
                existingTags.forEach(tag => {
                    const id = tag.getAttribute('data-id');

                    // Cập nhật trạng thái hiển thị của check icon
                    options.forEach(opt => {
                        if (opt.getAttribute('data-id') === id) {
